...
func GetgRPCSrv(ctx context.Context, param string) (*grpc.Server, error) {
	var server BackendServer
	var storage *db.DBStorage
	var err error
	storage, err = db.NewDBStorage(ctx, param)
	if err != nil {
		return nil, fmt.Errorf("getgrpcsrv: error when create storage %w", err)
	}
